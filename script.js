console.log(1)
document.getElementById("document-btn").addEventListener("click", function() {
    window.open("https://docs.google.com/document/d/1VakpU3LTmTrxoBdm2koGrIk3R2XVvExueMmLKw-_DjM/edit", "_blank");
});

document.getElementById("feedback-btn").addEventListener("click", function() {
    window.open("https://forms.gle/4Hpr5WDodHn5e1US7", "_blank");
});

document.getElementById("game-btn").addEventListener("click", function() {
    window.open("https://viktoriabashinska.github.io/Phaser2stGame/", "_blank");
});
